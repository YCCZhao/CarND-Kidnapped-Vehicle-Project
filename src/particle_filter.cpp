...
void ParticleFilter::dataAssociation(double sensor_range, std::vector<Map::single_landmark_s> landmarks, std::vector<LandmarkObs> observations) {
	...
	for (int k=0; k<particles.size(); k++) {
		...
		for (int i=0; i<landmarks.size(); i++) {
			...
			landmark_x = landmarks[i].x_f;
			landmark_y = landmarks[i].y_f;
			
			distance = dist(p_x, p_y, landmark_x, landmark_y);
			
			if (distance <= sensor_range) {
				landmarks_in_range.push_back(landmarks[i]);
			}
		}
		...
		for (int i=0; i<observations.size(); i++) {
			...
			landmark_x = landmarks_in_range[0].x_f;
			landmark_y = landmarks_in_range[0].y_f;
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	dataAssociation(sensor_range, map_landmarks.landmark_list, observations);
	...
	for (int k=0; k<particles.size(); k++) {
		...
		for (int i=0; i<particles[k].associations.size(); i++) {
			ob_x = particles[k].sense_x[i];
			ob_y = particles[k].sense_y[i];
			
			mu_x = map_landmarks.landmark_list[particles[k].associations[i]].x_f;
			mu_y = map_landmarks.landmark_list[particles[k].associations[i]].y_f;
